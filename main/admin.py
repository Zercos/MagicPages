...
class ReportingColoredAdminSite(ColoredAdminSite):
    ...
    def orders_per_day(self, request):
        ...
        order_data = (
            models.Order.objects.filter(date_added__gt=starting_day).annotate(day=TruncDay('date_added')).values(
                'day').annotate(c=Count('id'))
        )
    ...
    def most_bought_products(self, request):
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                data = (models.OrderLine.objects.filter(order__date_added__gt=starting_day).values(
                    'product__name').annotate(c=Count('id')))
